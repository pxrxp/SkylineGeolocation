...
from src.horizon_format import encode_horizon_uint8
...
class SkylineDatabaseGenerator:
    ...
    def generate_database(
        ...
        with rasterio.open(self.dem_file) as src:
            dem_crs = src.crs
            dlon_buf, dlat_buf = self._meters_to_degrees(
                self.dist_search_km * 1000.0, mean_lon, mean_lat
            )
            ...
            row_end = int(
                min(src.height, np.ceil(raw_window.row_off + raw_window.height))
            )
            col_start = int(max(0, np.floor(raw_window.col_off)))
            col_end = int(
                min(src.width, np.ceil(raw_window.col_off + raw_window.width))
            )
            ...
            x_mesh_dem, y_mesh_dem = np.meshgrid(
            ...
            vert_grid = hray.auxiliary.rearrange_pad_buffer(
                x_mesh.astype(np.float32), y_mesh.astype(np.float32), dem_data
            )
        ...
        xs_proj, ys_proj = to_utm_crs.transform(
            lon_loc[start_idx:end_idx], lat_loc[start_idx:end_idx]
        )

        to_dem_crs = Transformer.from_crs("EPSG:4326", dem_crs, always_xy=True)
        xs_dem_coords, ys_dem_coords = to_dem_crs.transform(
            lon_loc[start_idx:end_idx], lat_loc[start_idx:end_idx]
        )
        ...
        try:
            ...
            for i in tqdm(
                range(0, target_count, batch_size), desc="Ray-casting Skyline Grid"
            ):
                chunk_end = min(i + batch_size, target_count)
                chunk_size = chunk_end - i

                coords = np.column_stack(
                    (xs_proj[i:chunk_end], ys_proj[i:chunk_end], zs_proj[i:chunk_end])
                ).astype(np.float32)
                ...
                for idx in range(chunk_size):
                    ...
                    if save_raw_horizon:
                        ...
                        rec["raw_horizon_deg"] = encode_horizon_uint8(
                            raw_curve
                        ).tolist()
                ...
                if batch_records:
                    batch_table = pa.Table.from_pandas(
                        pd.DataFrame(batch_records), preserve_index=False
                    )
                    if writer is None:
            ...
            print(
                f"✓ Saved {written_count:,} viewpoints (skipped {skipped_flat_count:,} flat/unshielded sites) to {output_path}"
            )
        finally:
